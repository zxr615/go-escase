...
func Article(size int) string {
	...
	// 随机获取一个开始的位置
	st := rand.Intn(textLen)
	// 开始位数不够截取，向前补位
	repair := textLen - (st + size)
	if repair < 0 {
		st += repair
		return string(runeText[st:])
	}

	a := runeText[st : st+size]
	r := string(a)
	return r
}

// 孔乙己
func kongYiJi() string {
	return `鲁镇的酒店的格局，是和别处不同的：都是当街一个曲尺形的大柜台，柜里面豫备着热水，可以随时温酒。做工的人，傍午傍晚散了工，每每花四文铜钱，买一碗酒，——这是二十多年前的事，现在每碗要涨到十文，——靠柜外站着，热热的喝了休息；倘肯多花一文，便可以买一碟盐煮笋，或者茴香豆，做下酒物了，如果出到十几文，那就能买一样荤菜，但这些顾客，多是短衣帮，大抵没有这样阔绰。只有穿长衫的，才踱进店面隔壁的房子里，要酒要菜，慢慢地坐喝。
